...
let searchArtworksURL = "https://api.artic.edu/api/v1/artworks/search?q="
async function searchArtwork(query,size){
    searchArtworksURL += query + "&size=" + size
    const response = await fetch(searchArtworksURL)
    const data = await response.json()
    console.log(data)
    return data
}
